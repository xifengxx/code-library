...
export default function Detail({ data }) {
  ...
  return (
    <div className="detail">
      <Link to="/browse" className="back">← 返回浏览</Link>
      <header className="detail-head">
        <h1>{e.name}</h1>
        <div className="detail-links">
          {e.repo_url && (
            <a className="btn" href={e.repo_url} target="_blank" rel="noreferrer">GitHub 仓库 ↗</a>
          )}
          {(e.article_url || []).map((u, i) => (
            <a key={u} className="btn ghost" href={u} target="_blank" rel="noreferrer">相关文章 {i + 1} ↗</a>
          ))}
        </div>
        <div className="card-meta">
          <span className="badge cat">{LABELS[e.category] || e.category}</span>
          {e.language && <span className="badge">{e.language}</span>}
          {e.stars != null && <span className="badge star">⭐ {fmtStars(e.stars)}</span>}
          <span className={`badge status ${e.status}`}>{e.status}</span>
          {e.date_added && <span className="badge">📅 {e.date_added}</span>}
        </div>
